...
bool kek(size_t size, int value, vector<size_t> &first, vector<size_t> &second, vector<size_t> &boolean) {
    if (value <= 0) return true;
    oriental_graph g(2 * size);
    for (size_t i = 0; i < value; i++) {
        if (boolean[i] == 0) {
            g.add_edge(first[i] + size, second[i]);
            g.add_edge(second[i] + size, first[i]);
            g.add_edge(first[i], second[i] + size);
            g.add_edge(second[i], first[i] + size);
        } else {
            g.add_edge(first[i], second[i]);
            g.add_edge(second[i], first[i]);
            g.add_edge(first[i] + size, second[i] + size);
            g.add_edge(second[i] + size, first[i] + size);
        }
    }
    size_t k = 0;
    vector<size_t> scc = g.get_condensation(k);
    bool f = true;
    for (size_t i = 0; i < size; i++) {
        if (scc[i] == scc[i + size]) {
            f = false;
...
//    freopen("truth.out", "w", stdout);
    ...
    int n, k;
    cin >> n >> k;
    vector<size_t> aa(k, 0), bb(k, 0), cc(k, 0);
