...
class suffix_tree {
    ...
    public:
        ...
        const std::unordered_map<char, node*>& get_children() {
            return children;
        }
...
public:
    explicit suffix_tree(const std::string &word, int stop = -1) {
        this->word = word;
        this->child_nodes.reserve(2 * MAX_SIZE + 7);
    ...
    std::vector<std::pair<std::pair<int, int>, std::pair<int, int>>> to_vector() {
        ...
        std::unordered_map<node*, int> unordered_mapping;
        ...
        while (!q.empty()) {
            ...
            if (unordered_mapping.count(current) == 0) {
                unordered_mapping[current] = unordered_mapping.size();
            }
            if (current->is_leaf()) {
                continue;
            }
            for (const auto &child : current->get_children()) {
                if (unordered_mapping.count(child.second) == 0) {
                    unordered_mapping[child.second] = unordered_mapping.size();
                }
                result.push_back({{unordered_mapping[current], unordered_mapping[child.second]}, {child.second->get_start(), child.second->get_end()}});
                q.push(child.second);
            }
        }
        return result;
