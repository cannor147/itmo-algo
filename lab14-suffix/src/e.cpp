...
public:
    ...
    std::pair<long long, std::string> get_refrain() {
        auto lcp = get_lcp_for_suffices();

        std::stack<std::pair<std::pair<long long, long long>, int>> stack;
        long long max_value = word.length() - 1, max_size = 0, index = 0;
        for (int i = 0; i < word.length(); ++i) {
            long long x = 1;
            while (!stack.empty() && lcp[i] <= stack.top().first.second) {
                auto tmp = stack.top();
                stack.pop();

                x += tmp.first.first;
                if (x * tmp.first.second > max_value) {
                    index = tmp.second;
                    max_value = x * tmp.first.second;
                    max_size = tmp.first.second;
                }
            }
            stack.push({{x, lcp[i]}, i});
        }

        std::string result;
        if (max_value > word.length() - 1) {
            result = word.substr(values[index], max_size);
        } else {
            max_value++;
            result = word;
        }
...
int main() {
    ...
    for (int i = 0; i < n; ++i) {
        std::cin >> input[i];
        word += alphabet[input[i] - 1];
    }
    word += "$";

    suffix_array suf_array = suffix_array(word);
    auto refrain = suf_array.get_refrain();
    if (refrain.second.length() == n + 1) {
        refrain.first--;
    }
    std::cout << refrain.first << std::endl;
    std::cout << std::min((size_t)n, refrain.second.length()) << std::endl;
    for (int i = 0; i < std::min((size_t)n, refrain.second.length()); ++i) {
        std::cout << refrain.second[i] - 'a' + 1 << " ";
    }
}